...
export default function AdminSettingsClient({ initialConfig, initialEquipmentTypes }: AdminSettingsClientProps) {
    ...
    const handleRunDailyAutomation = async () => {
        if (!confirm('ต้องการสั่งรันระบบงานอัตโนมัติประจำวัน (ตรวจสอบค้างส่ง, เตือนใกล้กำหนด, เคลียร์คิวจอง) ทันทีหรือไม่?')) {
            return
        }
        ...
        try {
            ...
            if (res.success && res.result) {
                ...
                alert(res.result.summaryMessage)
            } else {
                alert(`เกิดข้อผิดพลาด: ${res.error || 'ไม่สามารถรันงานอัตโนมัติได้'}`)
            }
        } catch (err: any) {
                            ...
                            <section className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                                ...
                                <div className="space-y-3 text-sm text-gray-600">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-gray-50 rounded-xl gap-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => handleCopyToClipboard('/api/cron/daily-tasks')}
