            ...
            {mode === 'flowchart' && (
                <div className="py-2">
                    ...
                    <div className="flex flex-col items-center">
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 text-emerald-800 border-2 border-emerald-500 shadow-xs font-bold text-xs sm:text-sm ring-4 ring-emerald-50/60">
                        ...
                        {steps.map((step, idx) => {
                            ...
                            return (
                                <div key={step.stepNum} className="flex flex-col items-center">
                                    {/* Flowchart Node Box */}
                                    <div
                                        className={`w-full max-w-2xl bg-white rounded-2xl border-2 transition-all shadow-xs hover:shadow-md ${
                                            hasBranches
                                                ? 'border-amber-300 ring-4 ring-amber-50/60'
                                    ...
                                    >
                                        ...
                                        <div className="p-3.5 sm:p-4">
                                            <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
                                                <div className="flex items-center gap-2">
                                                    <span className="w-6 h-6 rounded-full bg-blue-600 text-white font-bold text-xs flex items-center justify-center shadow-2xs shrink-0">
                                                        {step.stepNum}
                                                    </span>
                                                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border flex items-center gap-1.5 ${actorStyle.bg} ${actorStyle.text} ${actorStyle.border}`}>
                                                        <span className={`w-1.5 h-1.5 rounded-full ${actorStyle.dot}`} />
                                                        {step.actor}
                                                    </span>
                                                </div>
                                                {step.statusBadge && (
                                            ...
                                            {hasBranches && (
                                                <div className="mt-3 pt-2.5 border-t border-amber-200/80 flex items-center gap-1.5 text-xs font-bold text-amber-800">
                                        ...
                                        {hasBranches && (
                                            <div className="bg-gradient-to-b from-amber-50/50 to-slate-50/80 p-3.5 sm:p-4 border-t border-amber-200/70 rounded-b-2xl">
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                                                    {step.branches!.map((branch, bIdx) => {
                                                        ...
                                                        return (
                                                            <div
                                                                key={bIdx}
                                                                className={`p-3 rounded-xl border-2 flex flex-col justify-between ${bStyle.bg} ${bStyle.border} shadow-2xs`}
                                                            >
                                                                <div>
                                                                    <div className="flex items-center justify-between gap-1.5 mb-1">
                                                                        <div className="flex items-center gap-1.5">
                                                                            {branch.type === 'success' && <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />}
                                                                            {branch.type === 'danger' && <XCircle className="w-4 h-4 text-rose-600 shrink-0" />}
                                                                            {branch.type === 'alert' && <AlertCircle className="w-4 h-4 text-amber-600 shrink-0" />}
                                                                            {branch.type === 'special' && <Sparkles className="w-4 h-4 text-indigo-600 shrink-0" />}
                                                                            <span className={`font-bold text-xs ${bStyle.text}`}>{branch.label}</span>
                                                                        </div>
                                                                        {branch.status && (
                                                                            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${bStyle.badge}`}>
                                                                                {branch.status}
                                                                            </span>
                                                                        )}
                                                                    </div>
                                                                ...
                                                                <div className="mt-2.5 pt-2 border-t border-black/5 flex items-center justify-between text-[11px] font-semibold text-slate-500">
                                                                    <span>ผลลัพธ์:</span>
                                                                    <span className={`inline-flex items-center gap-1 ${isTerminalExit ? 'text-rose-600' : 'text-emerald-700'}`}>
                                                                        {isTerminalExit ? (
                                                                            <>
                    ...
                    {/* END NODE */}
                    <div className="flex flex-col items-center">
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800 text-white border-2 border-slate-900 shadow-sm font-bold text-xs sm:text-sm ring-4 ring-slate-200">
                            <div className="w-5 h-5 rounded-full bg-slate-700 text-white flex items-center justify-center shrink-0">
                ...
                <div className="space-y-3.5 py-1">
                    {steps.map((step) => {
                        ...
                        return (
                            <div
                                key={step.stepNum}
                                className="bg-white rounded-2xl border border-slate-200 p-4 hover:border-slate-300 transition-colors shadow-2xs"
                            >
