...
export default function PendingApprovalClient({ initialProfile }: { initialProfile: any }) {
    const router = useRouter()
    const toast = useToast()
    const [profile, setProfile] = useState<any>(initialProfile)
    const [loggingOut, setLoggingOut] = useState(false)
    const previousStatusRef = useRef<string | null>(initialProfile?.status || null)
        ...
        const checkStatus = async () => {
            ...
            try {
                ...
                if (previousStatusRef.current && previousStatusRef.current !== data.status) {
                    ...
                    } else if (data.status === 'rejected' && previousStatusRef.current === 'pending') {
